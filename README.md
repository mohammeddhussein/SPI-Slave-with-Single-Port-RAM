...
1. rtl folder:
	+ SPI_TOP.v: top module of SPI
	+ RAM.v: design of RAM block
	+ ser.v: converting parallel bits to serial on MISO
 	+ deser.v: receiving serial bits on MOSI then converting it to parallel
    + FSM.sv: controlling states of the design
      
2. tb folder:
  	+ SPI_TB: testbench for covering most cases of SPI, using well-structured tasks
    + ser_tb: testbench for serializer debugging
      
+ ram.hex: which represents the memory for the testbench
+ run.do: do file to simulate QuestaSim/ModelSim
